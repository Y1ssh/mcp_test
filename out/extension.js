...
let wsServer;
let chatParticipant;
let languageModelToolsDisposables;
function activate(context) {
    ...
    try {
        ...
        languageModelToolsDisposables = (0, language_model_tools_1.registerLanguageModelTools)(mcpClient);
...
function registerCommands(context) {
    ...
    const commands = [
        ...
        },
        {
            ...
            handler: (message) => sendMessageToChat(message)
        },
        {
            name: 'mcp-bridge.trigger-agent',
            handler: (prompt) => triggerAutoAgent(prompt)
        },
        {
            name: 'mcp-bridge.get-history',
            handler: (limit) => getChatHistory(limit)
        },
        {
...
async function sendMessageToChat(message, context) {
    try {
...
async function startAICollaboration(task, strategy) {
    try {
