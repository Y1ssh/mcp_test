...
export class MCPChatParticipant {
     ...
     */
    private async handleChatCommand(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<void> {
     ...
     */
    private async handleAgentCommand(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<void> {
     ...
     */
    private async handleHistoryCommand(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<void> {
     ...
     */
    private async handleCollaborateCommand(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<void> {
     ...
     */
    private async handleDefaultCommand(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<void> {
     ...
     */
    public async sendMessageToCursorAI(message: string): Promise<void> {
        try {
            ...
            vscode.window.showInformationMessage(
                `Message ready to paste in chat: "${message.substring(0, 50)}..."`
            );
     ...
     */
    private async getChatHistory(limit: number = 10): Promise<any[]> {
        try {
     ...
     */
    private async startAICollaboration(task: string): Promise<void> {
        try {
     ...
     */
    private optimizePromptForAgent(task: string): string {
...
Please proceed with the implementation.`;
     ...
     */
    private async forwardToMCPServer(prompt: string): Promise<string> {
        try {
            ...
            return `❌ MCP server error: ${(error as Error).message}`;
        }
    }
